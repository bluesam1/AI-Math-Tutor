...
- [x] Task 1: Install Vision API Client Library (AC: 1, 7)
  - [x] Install OpenAI SDK (openai package) or Google Vision API client
...
- [x] Task 2: Create Image Upload Endpoint (AC: 1)
  - [x] Create functions/src/routes/problem.ts route file (note: path is functions/src/routes/problem.ts, not apps/api)
  - [x] Create POST /api/problem/parse-image endpoint
  - [x] Configure Express to handle file uploads (multer middleware)
  - [x] Set up file upload handling with size limits
  - [x] Register route in main Express app

- [x] Task 3: Implement File Validation (AC: 2)
  - [x] Create validation middleware for file format (JPG, PNG, GIF)
  - [x] Create validation middleware for file size (max 10MB)
  - [x] Validate file before processing
  - [x] Return appropriate error responses for invalid files

- [x] Task 4: Create Vision API Service (AC: 3, 7)
  - [x] Create functions/src/services/visionService.ts (note: path is functions/src/services/visionService.ts, not apps/api)
  - [x] Implement function to call Vision API with image
  - [x] Handle image conversion/formatting for API
  - [x] Extract text from Vision API response
  - [x] Use environment variable for API key (never hardcode)

- [x] Task 5: Implement Text Extraction Logic (AC: 3, 4)
  - [x] Process Vision API response to extract problem text
  - [x] Clean and format extracted text
  - [x] Return extracted text in API response
  - [x] Handle cases where no text is extracted

- [x] Task 6: Implement Error Handling (AC: 5)
  - [x] Handle Vision API network errors
  - [x] Handle Vision API authentication errors
  - [x] Handle Vision API parsing failures
  - [x] Return clear error messages to frontend
  - [x] Log errors for debugging (without exposing sensitive data)

- [x] Task 7: Implement Rate Limit Handling (AC: 6)
  - [x] Detect rate limit errors from Vision API
...
- [x] Task 8: Implement Problem Text Validation (AC: 8)
  - [x] Create validation function for extracted text
  - [x] Check if text is not empty
  - [x] Check if text contains math-related keywords or notation
  - [x] Return validation status in API response
  - [x] Handle cases where extracted text is not a math problem

- [x] Task 9: Create API Response Types (AC: 4)
  - [x] Create TypeScript types for API request/response
  - [x] Define response structure for successful parsing
  - [x] Define response structure for errors
  - [x] Export types for use in frontend (shared types)

- [x] Task 10: Configure Environment Variables (AC: 7)
  - [x] Add OPENAI_API_KEY to functions/.env.example (file creation blocked by gitignore, but documented in README)
  - [x] Document Vision API key setup in README
  - [x] Verify API key is loaded from environment
  - [x] Ensure API key is never exposed in client code
  - [x] Configure API key in CI/CD secrets (for deployment) - Firebase Functions secret configured

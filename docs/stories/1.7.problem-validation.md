...
- [x] Task 1: Install LLM API Client Library (AC: 2, 7)
  - [x] Install OpenAI SDK (openai package) or Anthropic SDK (anthropic package)
  - [x] Configure API client with environment variable for API key
  - [x] Verify API key is loaded from environment variables
  - [x] Create API client configuration module

- [x] Task 2: Create Problem Validation Endpoint (AC: 1)
  - [x] Create POST /api/problem/validate endpoint in problem routes
  - [x] Accept problem text in request body
  - [x] Validate request body structure
  - [x] Register endpoint in Express app

- [x] Task 3: Create LLM Service for Validation (AC: 2, 6, 7)
  - [x] Create functions/src/services/llmService.ts
  - [x] Implement function to call LLM API for problem validation
  - [x] Create prompt for problem validation
  - [x] Create prompt for problem type identification
  - [x] Parse LLM response to extract validation result
  - [x] Use environment variable for API key (never hardcode)

- [x] Task 4: Implement Problem Type Identification (AC: 3)
  - [x] Define problem type categories (arithmetic, algebra, geometry, word problems, multi-step)
  - [x] Create prompt to identify problem type
  - [x] Parse LLM response to extract problem type
  - [x] Validate problem type is one of the supported categories
  - [x] Return problem type in API response

- [x] Task 5: Implement Problem Validation Logic (AC: 2, 4)
  - [x] Create validation function using LLM API
  - [x] Check if problem text is a valid math problem
  - [x] Check if problem is appropriate for 6th grade level
  - [x] Return validation status (valid/invalid)
  - [x] Return cleaned problem statement if needed

- [x] Task 6: Create Validation Response Structure (AC: 4)
  - [x] Define TypeScript types for validation response
  - [x] Include validation status (valid/invalid)
  - [x] Include problem type (if valid)
  - [x] Include cleaned problem statement (if needed)
  - [x] Export types for use in frontend (shared types)

- [x] Task 7: Implement Error Handling (AC: 5)
  - [x] Handle invalid problems (non-math content)
  - [x] Handle unclear problems
  - [x] Return clear, age-appropriate error messages
  - [x] Handle LLM API errors (network, authentication, parsing)
  - [x] Log errors for debugging (without exposing sensitive data)

- [x] Task 8: Implement Rate Limit Handling (AC: 8)
  - [x] Detect rate limit errors from LLM API
  - [x] Return appropriate error message for rate limits
  - [x] Log rate limit occurrences
  - [ ] Consider implementing retry logic with exponential backoff (optional)

- [x] Task 9: Create Validation Prompts (AC: 6)
  - [x] Design prompt for problem validation
  - [x] Design prompt for problem type identification
  - [x] Test prompts with sample problems
  - [x] Refine prompts based on LLM responses
  - [x] Document prompt structure

- [x] Task 10: Configure Environment Variables (AC: 7)
  - [x] Add LLM API key to functions/.env.example (documented in README)
  - [x] Document LLM API key setup in README
  - [x] Verify API key is loaded from environment
  - [x] Ensure API key is never exposed in client code
  - [x] Configure API key in CI/CD secrets (for deployment)

- [x] Task 11: Integrate with Frontend (AC: 4)
  - [x] Create frontend API client function for problem validation
  - [x] Update ProblemInput component to call validation endpoint
  - [x] Handle validation responses (success and error)
  - [x] Update problem state with validation result
  - [x] Display validation errors if problem is invalid
